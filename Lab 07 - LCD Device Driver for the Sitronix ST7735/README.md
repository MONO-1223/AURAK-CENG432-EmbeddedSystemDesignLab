...
If `LCD_OutDec` and `LCD_OutFix` are implemented recursively, the stack plays a crucial role in managing function calls. Regardless of implementation choice, more registers than R0 - R3 will likely be needed, requiring push/pop operations to preserve register states. To ensure a structured approach to stack usage, local variables should be allocated symbolically rather than using hardcoded addresses. To maintain compatibility with future C function calls (in Labs 8, 9, and 10), data must be passed into functions using Register R0. Additionally, if registers R4 - R11 are used, they must be saved and restored to prevent unintended modifications across function calls. This adherence to register conventions ensures smooth interoperability between assembly and C-based driver components.

This lab will implement a "busy-wait" synchronization method, where the software waits for the LCD display to be ready before issuing an output command. Specifically, the system will ensure that the previous LCD command has been completed before proceeding. For the 10ms wait function, a cycle-counting approach, similar to Lab 2, is recommended instead of using the SysTick timer (as in Lab 5) because SysTick periodic interrupts will be required in Labs 8, 9, and 10.
...
__11. Where are we supposed to use the 10ms Wait function?__ <br> `delay10ms` will be used in `IO_Touch` to debounce your switches. You can call the function with a `BL` instruction.  <br> <br>
