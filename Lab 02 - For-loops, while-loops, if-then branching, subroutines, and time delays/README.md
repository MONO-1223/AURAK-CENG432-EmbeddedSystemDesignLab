...
Time is very important to embedded systems. As engineers, we rely heavily on understanding the execution times of software instructions to ensure optimal system performance. One effective method we employ is measuring the execution time of each instruction, a technique crucial for developing efficient embedded software. To achieve this on our [Tiva C LaunchPad (TM4C123) microcontroller](Photos/TM4C123GXL.png), we begin by utilizing the ARM data sheet—as the TM4C123 microcontroller is powered by an ARM Cortex-M4 core (CPU)—which provides detailed specifications essential for calculating execution times accurately. Since it is impossible to get an accurate time value using the cycle counting method, another more accurate technique to measure time uses a logic analyzer or oscilloscope. In the simulator, we will use a simulated logic analyzer, and on the real board, we will use an oscilloscope. To measure execution time, we cause rising and falling edges (LED toggles) on a digital output pin that occur at known places within the software execution. We can then use the logic analyzer or oscilloscope to measure the elapsed time between the rising and falling edges. 
...
From the three GIFs above, we can observe that the LED toggling becomes progressively slower with each successive example. This change is a result of the increasing delay time we designed into the program. As the delay value grows, the time between toggles becomes longer, causing the LED to turn on and off at a slower rate. This behavior clearly demonstrates the impact of the delay duration on the overall speed of the toggling process. Given that the clock period is 62.5 ns (which is the inverse of a 16 MHz clock), and the SUBS instruction takes 1 cycle, while the BNE instruction takes between 1 to 4 cycles depending on pipeline refilling, we can calculate the minimum and maximum time for a loop that iterates 400,000 times. For the minimum time, we assume that each BNE instruction takes the least number of cycles, which is 1 cycle. Therefore, each loop iteration would take 1 cycle for SUBS and 1 cycle for BNE, totaling 2 cycles per iteration. Since the loop runs 400,000 times, the total number of cycles would be 400,000 × 2 = 800,000 cycles. To find the time, we multiply the number of cycles by the clock period: 800,000 cycles × 62.5 ns/cycle = 50 ms. For the maximum time, we assume that each BNE instruction takes the maximum of 4 cycles. Therefore, each loop iteration would take 1 cycle for SUBS and 4 cycles for BNE, totaling 5 cycles per iteration. The total number of cycles for the loop would then be 400,000 × 5 = 2,000,000 cycles. Multiplying by the clock period gives us the maximum execution time: 2,000,000 cycles × 62.5 ns/cycle = 125 ms. Thus, the minimum time is 50 ms and the maximum time is 125 ms for 400,000 iterations of the loop. If the loop is intended to achieve a 100 ms delay with 400,000 iterations, we can calculate the tolerance based on the minimum and maximum times we previously found. Tolerance (min)=100ms−50ms=50ms and Tolerance (max)=125ms−100ms=25ms. Therefore, the tolerance range for the delay based on the minimum and maximum times would be ±50 ms, which is quite close and explains the given ±40 ms tolerance in the experiment's instructions! We have calculated and recorded the minimum and maximum time required to execute the LED toggling operation under different delay configurations. By adjusting the delay value, we were able to measure the time it takes for the LED to toggle between on and off states. The minimum time corresponds to the shortest delay, where the toggling happens at the fastest possible rate, while the maximum time reflects the longest delay, resulting in the slowest toggling. Finally, we have learned that 10 seconds in real life are equal to [3.28 seconds](Photos/simulation-time.png) in simulation time. Simulators typically run slower than real hardware. We used the built-in logic analyzer to measure how much Cortex-M time is simulated in 10 human seconds by hitting the reset twice to clear the time axis on the plot, running the simulator for 10 human seconds (real time measured with a stopwatch), and then stopping the simulation. There are many factors that affect this ratio, so it is expected to vary. The point of the exercise is to get a sense of how a simulator manages time. 
...
We can practice modifying this lab in several ways to explore different functionalities, improve our design, and cement our understanding. One approach is to use a different switch, such as SW2, instead of SW1 to control the LED toggling. Another variation could involve toggling a different color LED than the green one, perhaps using the red or blue LEDs on the same port. Additionally, we could experiment with designing the system to handle different delay values, creating slower or faster toggling effects based on the delay input. For a more advanced modification, we could combine all of these elements into a single program, where one switch controls the toggling of one color LED at a certain delay and the other switch controls another color LED at a different delay. This would allow for more dynamic control of multiple LEDs with varying timing, offering further flexibility and complexity in the design.
...
gantt
    ...
    axisFormat %a-%Y-%m-%d
    section Preparation         
        Nour Mostafa : crit, 2025-01-30 00:00, 02h
        Mohamed Abouissa : crit, 2025-01-30 00:00, 02h
    section Keil         
        Mohamed Abouissa : crit,2025-01-31 00:00, 1d
    section Results       
        Nour Mostafa : crit, 2025-01-31 00:00, 1d
    section Report
        Nour Mostafa : crit, 2025-02-8 00:00, 2d
        Mohamed Abouissa : crit, 2025-02-7 00:00, 2d
```
