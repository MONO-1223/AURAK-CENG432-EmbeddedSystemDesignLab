...
For an overview of the oscilloscope's use cases check this [infographic](Photos/oscilloscope.png). Note that some oscilloscopes come with a COM channel which must be connected to the ground of the circuit (as in the schema) and some oscilloscopes don't come with a COM channel and instead have a fork in their probe wires for every channel that must be connected to the ground (as in the demo gif).
